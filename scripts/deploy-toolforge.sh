...
if [[ "$www" -eq 1 ]]; then
  ...
  cp "$d/ne_10m_capitals.geojson" "$stage/data/ne_10m_capitals.geojson"
  # Points of view, as differences from the de facto countries.
  python3 "$here/scripts/worldview-diffs.py" "$d/ne_10m_admin_0.geojson" "$stage/data"
